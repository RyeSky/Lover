...
//        hideSoftInput();
    ...
    public boolean onTouch(View v, MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_UP && fc.isShown()) {
            ...
            hideSmileLayout(true);//隐藏表情布局，显示软键盘
            //软键盘显示后，释放内容高度
            content.postDelayed(this::unlockBodyHeightDelayed, 200L);
        }
        return false;
    }
    ...
    private void lockBodyHeight() {
        LayoutParams params = (LayoutParams) body.getLayoutParams();
    ...
    private void unlockBodyHeightDelayed() {
        content.postDelayed(() -> ((LayoutParams) body.getLayoutParams()).weight = 1.0F, 200L);
    }
