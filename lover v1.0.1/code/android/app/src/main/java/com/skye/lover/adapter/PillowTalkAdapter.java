...
public class PillowTalkAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        //头像
        vh.avatarOne.getHierarchy().setFailureImage(CommonUtil.getFailureImage(pt.getPublisherGender()));
        if (!TextUtils.isEmpty(pt.getPublisherAvatar())) {
            AvatarTag tag = pt.getPublisherAvatarTag();
            if (!tag.equals(vh.avatarOne.getTag())) {
                vh.avatarOne.setImageURI(URLConfig.SERVER_HOST + tag.avatar);
                vh.avatarOne.setTag(tag);
            }
        } else if (vh.avatarOne.getTag() == null || !ConstantUtil.APPNAME.equals(vh.avatarOne.getTag())) {
            vh.avatarOne.setImageURI("");
            vh.avatarOne.setTag(ConstantUtil.APPNAME);
        }
        ...
        if (pt.getType() == PillowTalk.PILLOW_TALK) {//悄悄话
            vh.avatarAnother.getHierarchy().setFailureImage(CommonUtil.getFailureImage(pt.getAnotherGender()));
            if (!TextUtils.isEmpty(pt.getAnotherAvatar())) {
                AvatarTag tag = pt.getAnotherAvatarTag();
                if (!tag.equals(vh.avatarAnother.getTag())) {
                    vh.avatarAnother.setImageURI(URLConfig.SERVER_HOST + tag.avatar);
                    vh.avatarAnother.setTag(tag);
                }
            } else if (vh.avatarAnother.getTag() == null || !ConstantUtil.APPNAME.equals(vh.avatarAnother.getTag())) {
                vh.avatarAnother.setImageURI("");
                vh.avatarAnother.setTag(ConstantUtil.APPNAME);
            }
        ...
        if (!TextUtils.isEmpty(img)) {
            ...
            if (!img.equals(vh.img.getTag())) {
                vh.img.setImageURI(URLConfig.SERVER_HOST + img);
                vh.img.setTag(img);
            }
    ...
    static class ViewHolder {
        ...
        SimpleDraweeView avatarOne;//相恋关系中的一方头像
        @BindView(R.id.avatar_another)
        SimpleDraweeView avatarAnother;//相恋关系中的另一方头像
        @BindView(R.id.img)
        SimpleDraweeView img;//悄悄话图片
        ...
        ViewHolder(View view) {
            ...
            hierarchy.setFailureImage(view.getResources().getDrawable(R.drawable.shape_empty));
        }
    }
}
