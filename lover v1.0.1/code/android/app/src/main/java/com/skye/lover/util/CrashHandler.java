...
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    public void init(Context context) {
        ...
        if (isCrashed()) {
            ...
            Cross cross = OkHttpUtil.buildPostCall(URLConfig.ACTION_SUBMIT_BUG, params, false, false);
            if (cross.isNetworkConnected)
                OkHttpUtil.executePostCall(cross).subscribeOn(Schedulers.io())
                        .observeOn(AndroidSchedulers.mainThread())
                        .subscribe(
                                new QuietResponseSubscriber<String>() {
                                    @Override
    ...
    private void save(Throwable ex) {
        ...
        String bug = formatter.format(new Date()) + "\n" + exs;
        CommonUtil.log(bug);
        try {// 保存bug成文件
