...
public class ActionSheet extends Dialog implements OnClickListener {
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.bg_view && !mCancelableOnTouchOutside)
            return;
        dismissMenu();
        if (v.getId() != R.id.btn_cancel && v.getId() != R.id.bg_view) {
            if (mListener != null)
                mListener.onItemClick(v.getId() - R.id.btn_cancel - 1);
            isCancel = false;
        }
    }
