...
public class DynamicAddPictureContainer extends RelativeLayout implements View.OnClickListener {
        ...
        for (int i = 0; i < 12; i++) {
            if (i < size) {//需要显示
                ...
                if (image.type == 0) {//图标
                    ...
                    imgs[i].setImageResource(R.drawable.shape_empty);
     ...
     * 显示图片
     */
    private void displayImage(Image image, ImageView img) {
        if (!TextUtils.isEmpty(image.filePath)) {
            if (!image.equals(img.getTag())) {
                ImageLoader.getInstance().displayImage("file://" + image.filePath, img);
                img.setTag(image);
            }
        } else {
            img.setImageResource(R.drawable.shape_white);
            img.setTag(null);
        }
    }
