 ...
 * 发表评论界面
 */
public class PublishCommentActivity extends BaseActivity implements View.OnTouchListener, FaceContainer.OnFaceOprateListener {
     ...
     * 发表评论接口回调
     */
    private Callback callbackPublishComment = new Callback() {
        @Override
            ...
            handler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        if (br.check()) {
                            CommonUtil.toast(context, R.string.comment_success);
                            setResult(RESULT_OK);
                            finish();
                        } else
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        initTopbar();
        topbar.title.setText(R.string.comment);
        ...
        switch (v.getId()) {
            case R.id.btn_ok://确定
                ...
                if (TextUtils.isEmpty(content)) {
                    CommonUtil.toast(context, R.string.comment_can_not_empty);
                    return;
                }
                ...
                OkHttpUtil.doPost(context, URLConfig.ACTION_PUBLISH_COMMENT, params, callbackPublishComment);
                break;
